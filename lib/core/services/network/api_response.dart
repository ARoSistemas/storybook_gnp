...
///
/// The `ApiResponse` class is a generic wrapper that holds the outcome of
/// an API operation. It can represent either a successful result or a failure.
/// This class helps in managing and handling responses from API calls more
/// effectively by providing a unified way to handle both success and failure
/// cases.
///
...
///
/// **Properties**:
/// - `failure`: An instance of `Failure` representing the error result,
/// or `null` if the operation was successful.
/// - `success`: An instance of `Success` representing the successful result,
/// or `null` if the operation failed.
/// - `isFailure`: A boolean indicating whether the result represents
/// a failure (`true`) or a success (`false`).
///
...
///    Executes the appropriate function based on whether the result is a
///    failure or a success.
///
///   - `failure`: A function to handle the failure case,
///      taking a `Failure` parameter and returning a value of type `T`.
///
///   - `success`: A function to handle the success case,
///      taking a `Success` parameter and returning a value of type `T`.
///
///   - Returns the result of the function that matches the current state
///     (`failure` or `success`).
///
/// **Example**:
/// ```dart
...
///     ); // Output: Success: 42
///
///     ApiResponse<String, int> errorResult =
///         ApiResponse.failure('An error occurred');
///
///     errorResult.when(
///       (failure) => print('Failure: $failure'),
///       (success) => print('Success: $success'),
...
class ApiResponse<Failure, Success> {
  ...
  factory ApiResponse.failure(Failure failure) =>
      ApiResponse._(failure, null, true);
  ...
  factory ApiResponse.success(Success success) =>
      ApiResponse._(null, success, false);
